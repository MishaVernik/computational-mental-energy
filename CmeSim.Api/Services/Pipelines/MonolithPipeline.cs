...
/// <summary>
/// Architecture A: Monolith pipeline - all operations synchronous in API.
/// </summary>
public class MonolithPipeline : IInferencePipeline
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Stage 2: Preprocess (simulated - in real system this would extract features)
        var preprocessStart = DateTime.UtcNow;
        // Simulate preprocessing delay
        if (context.Config.NetworkProfile.MeanMs > 0)
        {
            var delay = SimulateDelay(context.Config.NetworkProfile.MeanMs, context.Config.NetworkProfile.StdMs, context.Config.Seed);
        ...
        var cmeCalcResult = _cmeCalculator.ComputeCme(request.Features, quantumResult.PFlow, request.TaskDifficulty);
