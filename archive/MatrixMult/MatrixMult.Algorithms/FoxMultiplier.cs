...
/// <summary>
/// Fox's algorithm for parallel matrix multiplication.
/// Uses a p x p process grid where p = floor(sqrt(threads)).
/// In each phase, broadcasts A blocks along rows, then performs local multiply.
/// </summary>
public class FoxMultiplier : IMatrixMultiplier
{
    public MultiplyResult Multiply(Matrix a, Matrix b, Matrix c, int blockSize, int maxDegreeOfParallelism)
    {
        ...
        // Simulate communication: broadcast A blocks along rows in each phase
        ...
        // Fox algorithm: numBlocks phases (one per k-block)
        // In each phase, we broadcast A blocks along rows, then multiply with B blocks
        for (int phase = 0; phase < numBlocks; phase++)
        {
            // Simulate broadcast overhead
            commStart = DateTime.UtcNow;
            // In real MPI: broadcast A[i, phase] to all processes in row i
            SimulateBroadcast(p);
            ...
            Parallel.For(0, actualWorkers, parallelOptions, workerId =>
            {
                ...
                for (int i = gridRow; i < numBlocks; i += p)
                {
                    ...
                    {
                        // In Fox: A[i, phase] * B[phase, j] contributes to C[i, j]
                        int blockRow = i * blockSize;
                        int blockCol = j * blockSize;
                        int kStart = phase * blockSize;
    ...
    /// <summary>
    /// Simulates broadcast communication overhead.
    /// In real MPI, this would be a broadcast operation.
    /// </summary>
    private void SimulateBroadcast(int p)
    {
        // Simulate communication delay: O(log p) for tree-based broadcast
        int steps = (int)Math.Ceiling(Math.Log2(p + 1));
        for (int i = 0; i < steps; i++)
        {
            // Simulate network latency (very small overhead)
            Thread.SpinWait(100 * steps);
        }
    }
}
